  ...
  return (
    <div className="group relative w-full mt-8">
      {showLeftBtn && (
        <button
          onClick={() => scroll("left")}
          className="absolute left-[-20px] top-1/2 z-10 -translate-y-1/2  bg-black/20 text-[#000000] border border-black-200 p-2 rounded-full shadow-lg hover:bg-slate-50 transition-all"
        >
          <ChevronLeft size={24} />
        </button>
      )}
      ...
      >
        {placements.map((placement, index) => (
          <PlacementCard props={placement} key={index} />
        ))}
      </div>
      ...
      {showRightBtn && (
        <button
          ...
          className="absolute right-[-20px] top-1/2 z-10 -translate-y-1/2 bg-black/20 text-[#000000] border border-black-200 p-2 rounded-full shadow-lg hover:bg-slate-50 transition-all"
        >
...
export function PlacementCard({ props }: { props: PlacementProp }) {
  return (
    <div className="flex flex-col min-w-[400px] min-h-[260px] p-2.5 bg-[#FFFFFF] border-1 border-[#E2E8F0] rounded-xl">
      <div className={`flex flex-col gap-3.5 ${props.color} p-2.5 rounded-xl`}>
        ...
        <div className="flex gap-8 px-2.5">
          {props.tags.map((tag, index) => (
            <span
              key={index}
              className="border-1 border-[#000000] rounded-4xl text-[10px] text-[#0F172A] leading-5 py-1 px-6 h-8 text-center"
            >
              {tag}
            </span>
          ))}
